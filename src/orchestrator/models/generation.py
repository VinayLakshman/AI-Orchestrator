"""Provider-neutral model generation response helpers.

The orchestrator speaks to an OpenAI-compatible router, so these types are
not specific to Ollama. ``models.ollama`` remains a compatibility facade.
"""
...
def extract_assistant_text(value: Any) -> str:
    """Normalize a provider response down to user-visible assistant text."""
    if value is None:
        return ""
    ...
    if isinstance(value, dict):
        for key in ("content", "response", "text", "message", "choices", "messages"):
            if key in value:
